...
class MSDBlock(nn.Module):
    def __init__(self, norm='weight'):
        ...
        self.convs = nn.ModuleList([
            ...
            self.norm(nn.Conv1d(128, 128, kernel_size=41, stride=2, groups=4, padding=20)),
            self.norm(nn.Conv1d(128, 256, kernel_size=41, stride=2, groups=16, padding=20)),
            self.norm(nn.Conv1d(256, 512, kernel_size=41, stride=4, groups=16, padding=20)),
            self.norm(nn.Conv1d(512, 1024, kernel_size=41, stride=4, groups=16, padding=20)),
            self.norm(nn.Conv1d(1024, 1024, kernel_size=41, stride=1, groups=16, padding=20)),
    ...
    def forward(self, x):
        ...
        for i in range(len(self.convs) - 1):
            x = F.leaky_relu(self.convs[i](x), TaskConfig.leaky)
            res.append(x)
        out = self.convs[-1](x)
        res.append(out)
        return res
